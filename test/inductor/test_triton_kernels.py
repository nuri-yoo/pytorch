...
class KernelAccessAnalyzerTests(torch._inductor.test_case.TestCase):
    ...
    def test_matmul():
        ...
        return (
            ...
            [
                UserTritonDep(
                    name="a_ptr",
                    index=BLOCK_SIZE_M * stride_am * sympy.Mod(i0, GROUP_SIZE_M)
                    + i3 * stride_am
                ...
                UserTritonDep(
                    name="b_ptr",
                    index=sympy.floor(i0 / GROUP_SIZE_M) * BLOCK_SIZE_N * stride_bn
                    + i7 * stride_bn
            ...
            [
                UserTritonDep(
                    name="c_ptr",
                    index=sympy.Mod(i0, GROUP_SIZE_M) * BLOCK_SIZE_M * stride_cm
                    + i1 * stride_cm
                    + sympy.floor(i0 / GROUP_SIZE_M) * BLOCK_SIZE_N * stride_cn
                    + i2 * stride_cn,
                    var_names=(i0, i1, i2),
    ...
    def test_negative_offset():
        ...
        @triton.jit
        def negative_offset_kernel(a_ptr, b_ptr, N, BLOCK_SIZE: tl.constexpr):
            pid = tl.program_id(0)
            offsets = tl.arange(0, BLOCK_SIZE)
            # ptr starts at end of block, reads backwards
            base = (pid + 1) * BLOCK_SIZE - 1
            a = tl.load(a_ptr + base - offsets)
            tl.store(b_ptr + base - offsets, a)
